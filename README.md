# www.gigshare.live static site

Minimal static pages for the public domain (legitimacy, contact, Stripe Connect handoff).

## Stripe Connect (backend env)

Set these on the **API** (not in this folder) to match the deployed URLs exactly:

- `STRIPE_CONNECT_ONBOARDING_RETURN_URL` → `https://www.gigshare.live/stripe-connect/return/`
- `STRIPE_CONNECT_ONBOARDING_REFRESH_URL` → `https://www.gigshare.live/stripe-connect/refresh/`
...
4. Attach custom domain **www.gigshare.live** (and apex if needed) in Pages → Custom domains.
5. After deploy, verify:
   - `https://www.gigshare.live/`
   - `https://www.gigshare.live/stripe-connect/return/`
   - `https://www.gigshare.live/stripe-connect/refresh/`
